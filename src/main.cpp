...
void setup()
{
  ...
  if (!communication.beginRadioOne(config.radio_config_one))
  {
    while (true)
    {
      Serial.println("Configuring radio one failed");
      delay(5000);
    }
  }
  ...
  if (!communication.beginRadioTwo(config.radio_config_two))
  {
    while (true)
    {
      Serial.println("Configuring radio two failed");
      delay(5000);
    }
  }
...
void loop()
{
  ...
  if (communication.receiveRadioOne(msg, msg_length, rssi, snr, frequency))
  {
    ...
    if (checksum_good)
    {
      ...
      Serial.println("PAYLOAD TELEMETRY PACKET");

      // Serial.print("PAYLOAD TELEMETRY PACKET | HEX: ");
      // // Print as hex
  ...
  if (communication.receiveRadioTwo(msg_2, msg_length, rssi, snr, frequency))
  {
    ...
    if (checksum_good)
    {
      ...
      // Serial.print("BALLOON TELEMETRY PACKET | HEX: ");
      // // Print as hex
      // for (size_t i = 0; i < msg_length; i++)
      // {
      //   Serial.print(msg_2[i], HEX);
      //   Serial.print(" ");
      // }
      // Serial.println();
